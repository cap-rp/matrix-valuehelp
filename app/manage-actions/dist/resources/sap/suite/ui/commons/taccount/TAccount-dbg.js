...
sap.ui.define([
...
], function (library, Device, Control, ItemNavigation, TAccountGroup, TAccountPanel, IconPool, Button, InvisibleText, encodeXML, TAccountUtils, Utils, BigNumber, UI5Date, CoreLib) {
	"use strict";
			...
			render: function (oRm, oAccount) {
				...
				var fnWriteOrderedColumn = function (aItems) {
					...
					aItems.sort(function (oItem1, oItem2) {
						return UI5Date.getInstance(oItem1._sOrderBy) >= UI5Date.getInstance(oItem2._sOrderBy) ? 1 : -1;
					});
	...
	TAccount.prototype._getDownArrow = function () {
		...
		if (!this._oArrowDown) {
			this._oArrowDown = new Button({
				type: "Transparent",
				tooltip: this._getExpandAltText(!bCollapsed),
				icon: bCollapsed ? "sap-icon://navigation-right-arrow" : "sap-icon://navigation-down-arrow",
				press: function () {
	...
	TAccount.prototype.setCollapsed = function (bValue) {
		...
		this._getDownArrow().setIcon(!bValue ? "sap-icon://navigation-down-arrow" : "sap-icon://navigation-right-arrow");
