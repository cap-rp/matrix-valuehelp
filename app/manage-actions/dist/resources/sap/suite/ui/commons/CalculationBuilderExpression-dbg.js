...
sap.ui.define([
	...
	const OperatorTooltip = Object.freeze({
		"+": "CALCULATION_BUILDER_ADDITION_TOOLTIP",
		"-": "CALCULATION_BUILDER_SUBTRACTION_TOOLTIP",
		"/": "CALCULATION_BUILDER_DIVISION_TOOLTIP",
		"*": "CALCULATION_BUILDER_MULTIPLICATION_TOOLTIP",
		"(": "CALCULATION_BUILDER_LEFT_BRACKET_TOOLTIP",
		")": "CALCULATION_BUILDER_RIGHT_BRACKET_TOOLTIP",
		",": "CALCULATION_BUILDER_COMMA_TOOLTIP",
		"<": "CALCULATION_BUILDER_LESS_THAN_TOOLTIP",
		">": "CALCULATION_BUILDER_GREATER_THAN_TOOLTIP",
		"<=": "CALCULATION_BUILDER_LESS_THAN_OR_EQUALS_TO_TOOLTIP",
		">=": "CALCULATION_BUILDER_GREATER_THAN_OR_EQUALS_TO_TOOLTIP",
		"=": "CALCULATION_BUILDER_EQUALS_TO_TOOLTIP",
		"!=": "CALCULATION_BUILDER_NOT_EQUALS_TO_TOOLTIP",
		"AND": "CALCULATION_BUILDER_LOGICAL_CONJUNCTION_TOOLTIP",
		"OR": "CALCULATION_BUILDER_LOGICAL_DISJUNCTION_TOOLTIP",
		"XOR": "CALCULATION_BUILDER_EXCLUSIVE_DISJUNCTION_TOOLTIP",
		"NOT": "CALCULATION_BUILDER_NEGATION_TOOLTIP"
	});
		...
		metadata: {
			...
			aggregations: {
				...
				items: {
					...
					defaultClass: CalculationBuilderItem,
					multiple: true,
					singularName: "item",
					bindable: "bindable"
				},
				...
				variables: {
					...
					defaultClass: CalculationBuilderVariable,
					multiple: true,
				...
				functions: {
					...
					defaultClass: CalculationBuilderFunction,
					multiple: true,
				...
				groups: {
					...
					defaultClass: CalculationBuilderGroup,
					multiple: true,
					singularName: "Group"
				}
			},
	...
	CalculationBuilderExpression.prototype.onsapspace = function (oEvent) {
		...
		// pressing the space and enter keys on the keyboard must perform the same action
		this._handleEnter(oEvent);
	};
	...
	CalculationBuilderExpression.prototype._createPopoverLayout = function (oPopoverItems) {
		var fnCreateButton = function (sLabel) {
			return new Button({
				text: sLabel,
				tooltip: OperatorTooltip[sLabel] ? oResourceBundle.getText(OperatorTooltip[sLabel]) : OperatorTooltip[sLabel],
			...
			Object.keys(oOperators).forEach(function (sKey) {
				...
				if (oParent._isTokenAllowed(sKey)) {
					...
					var oButton = new Button({
						text: sText,
						tooltip: OperatorTooltip[sText] ? oResourceBundle.getText(OperatorTooltip[sText]) : sText,
						press: this._updateOrCreateItem.bind(this, {
							type: sType,
							key: sItemKey
						})
			...
			onClose: function (sAction) {
				if (sAction === MessageBox.Action.YES) {
					var aItems = this.$().find(".sapCalculationBuilderSelected .sapCalculationBuilderItem"),
						...
						$first = aItems.first(),
						oItem = Element.getElementById($first.attr("id"));
	...
	CalculationBuilderExpression.prototype._getVariableByKey = function (sKey) {
		...
		return this._mVariableMap[sKey];
	};
	...
	CalculationBuilderExpression.prototype._createMap = function (sKey) {
		this._mVariableMap = {};
		this._mFunctionMap = {};
		this._mOperatorMap = {};
		this._setVariables();
		...
		for (var i = 0; i < aItems.length + 1; i++) {
			...
			if (iIndex === i) {
				$selectedItems.each(function () { // eslint-disable-line
					...
					if ($this.hasClass("sapCalculationBuilderItem")) {
						oItem = Element.getElementById(jQuery(this)[0].id);
	...
	CalculationBuilderExpression.prototype._setVariables = function () {
		var aVariables = this["getVariables"]();
		aVariables.forEach((oVariable)=> {
			this._mVariableMap[oVariable.getKey().toLowerCase()] = oVariable;
		});
	};

	CalculationBuilderExpression.prototype._setFunctions = function () {
		var aFunctions = this["getFunctions"]();
		aFunctions.forEach((oFunction)=> {
			this._mFunctionMap[oFunction.getKey().toLowerCase()] = oFunction;
		});
	};

	CalculationBuilderExpression.prototype._setOperators = function () {
		var aOperators = this["getOperators"]();
		aOperators.forEach((oOperator)=> {
			this._mOperatorMap[oOperator.getKey().toLowerCase()] = oOperator;
		});
	};
