...
sap.ui.define([
	...
	"sap/ui/core/Lib"
			 ...
			 TimelineRenderManagerTimestamp, StringSliderTooltip, ResponsiveScale, coreLibrary, CoreLib) {
	"use strict";
	...
	var TimelineRenderManager = {
		extendTimeline: function (Timeline) {
			...
			Timeline.prototype._registerResizeListener = function () {
				...
				if (oParent) {
					this.oResizeListener = ResizeHandler.register(oParent, this._performResizeChanges.bind(this));
				}
			};
			...
			Timeline.prototype._calculateTextHeight = function () {
				...
				if (sTextHeight) {
					...
					} else if (Number.isFinite(Number(sTextHeight))) {
			...
			Timeline.prototype._setMessageBars = function (ctrl) {
				...
				if (sMessage) {
					...
					this._objects.getFilterMessageText().setTooltip(sMessage);
				}
			};
