...
sap.ui.define([
	...
	"sap/ui/core/Lib"
], function (library, HtmlElement, DateFormat, Icon, encodeXML, encodeCSS, InvisibleText, UI5Date, CoreLib) {
	"use strict";
	...
	TimelineItemRenderer._getFormatedDateTime = function (oTimelineItem, mArguments) {
		...
		sAriaLabelForDateTime = sConvertedDate + " " + oResourceBundle.getText("TIMELINE_AT") + " " + this._formatTimeForAriaLabel(oTimeFormat.format(oDateTime));
		return sConvertedDate + " " + oResourceBundle.getText("TIMELINE_AT") + " " + oTimeFormat.format(oDateTime);
	};

	TimelineItemRenderer._formatTimeForAriaLabel = function(input) {
		const sNormalizedInput = input.replace(/\u202F/g, ' '); // Replace non-breaking space with regular space
		const [sHoursMinutes, sMeridian] = sNormalizedInput.split(" ");
		const [sHours, sMinutes] = sHoursMinutes.split(":").map(Number);

		// Construct the aria-label
		if (sMeridian){
			return input;
		} else {
			return `${sHours} hours : ${sMinutes} minutes`;
		}
	  };
	...
	TimelineItemRenderer._addAccessibilityTags = function (eTimelineItem, oTimelineItem, bIsGroupHeader) {
		...
		if (oTimeline.isA("sap.suite.ui.commons.Timeline") && oTimeline.getGroupByType() !== TimeLineGroupType.None && oTimeline.getAxisOrientation() === "Vertical") {
			//Setting the role as option would suppress the count of the focused item, We are setting like this to support backward compatability
