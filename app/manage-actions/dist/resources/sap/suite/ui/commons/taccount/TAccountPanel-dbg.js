...
sap.ui.define([
	...
	"sap/ui/core/Lib"
			 ...
			 ToolbarSpacer, Text, SegmentedButton, SegmentedButtonItem, FilterOperator, encodeXML, BaseObject, NumberFormat, Core, Configuration, TAccountUtils,
			 TAccountPanelRenderer, MobileLibrary, BigNumber, Element, Lib) {
	"use strict";
	...
	TAccountPanel.prototype.getSum = function () {
		...
		if (!this._oSum) {
			for (var i = 0; i < aContent.length; i++) {
				...
				if (BaseObject.isA(oGroup, "sap.suite.ui.commons.taccount.TAccountGroup")) {
		...
		Object.keys(oChangedProperties).forEach(function (sKey) {
			...
			aItems.each(function (i, oItem) {
				var bVisible = oProperty.getVisible(),
					oCurrentItem = Element.getElementById(oItem.id);
	...
	TAccountPanel.prototype._createPopover = function () {
		...
		this._oPopover.setEndButton(new Button({
			...
			press: function (oEvent) {
				...
				//updating the list to make sure only selected and saved items are checked
				aProperties.forEach((property,index)=>{
					oList.getItems()[index].setSelected(property.mProperties.visible)
				})
			}
	...
	TAccountPanel.prototype._createToolbar = function () {
		...
		var sToolbarTitle = this.getId() + "-toolbartitle",
			sToolbarBalanceLabel = this.getId() + "-toolbarbalancelabel",
			sToolbarSumTitle = this.getId() + "-toolbarsumtitle";

		this._oToolbar.addContent(new Title(sToolbarTitle, {
			text: this.getTitle()
		}));

		this._oToolbar.addContent(new ToolbarSpacer(this.getId() + "-toolbarspacer"));

		this._oToolbar.addContent(new Text(sToolbarBalanceLabel, {
			wrapping: false,
			text: oResourceBundle.getText("TACCOUNT_TOTALBALANCE") + ":"
		}).addStyleClass("sapSuiteUiCommonsAccountPanelLabel"));

		this._oToolbar.addContent(new Text(sToolbarSumTitle, {
			wrapping: false
		...
		this._oDisplaySwitcher = new SegmentedButton(this.getId() + "-toolbarswitcher", {
			selectionChange: function (oEvent) {
				var oItem = Element.getElementById(that._oDisplaySwitcher.getSelectedItem());
				if (oItem) {
			...
			ariaLabelledBy: [sToolbarBalanceLabel, sToolbarSumTitle]
		});
