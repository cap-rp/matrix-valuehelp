...
sap.ui.define([
...
], function (jQuery, library, MobileLibrary,ImageHelper, Control, ProcessFlowConnection, ProcessFlowLaneHeader,
		Device, ResizeHandler, Log, KeyCodes, ProcessFlowRenderer, UI5Date, Core, CoreLib, Element, AriaProperties, Localization) {
	"use strict";
	...
	var ProcessFlow = Control.extend("sap.suite.ui.commons.ProcessFlow", /** @lends sap.suite.ui.commons.ProcessFlow.prototype */ {
		metadata: {
			...
			aggregations: {
				 ...
				 */
				ariaProperties: {type: "sap.suite.ui.commons.AriaProperties", multiple: false, group: "Misc", defaultClass: AriaProperties}
			},
			events: {
				/**
	...
	ProcessFlow.prototype.init = function () {
		...
		if (!this._resBundle) {
			this._resBundle = CoreLib.getResourceBundleFor("sap.suite.ui.commons");
		}
		this._internalLanes = [];
	...
	ProcessFlow.prototype.onAfterRendering = function () {
		...
		if ($content && $content.length) {
			...
			if (this._headerHasFocus) {
				...
				if ($nodeToFocus[0]) {
					var oNodeToFocus = Element.getElementById($nodeToFocus[0].id);
	...
	ProcessFlow.prototype._onResize = function () {
		...
		if (!this._iLastResizeEventTime || ((iActualTime - this._iLastResizeEventTime) < 50)) {
			...
			if (!this._iLastResizeHandlingTime || (iActualTime - this._iLastResizeHandlingTime > 500)) { //Handle each .5s.
				this.onAfterRendering();
				this._iLastResizeHandlingTime = iActualTime;
			}
		} else {
			this._iLastResizeHandlingTime = null;
		}

		this._iLastResizeEventTime = iActualTime;
	};
	...
	ProcessFlow.prototype.onfocusin = function (oEvent) {
		...
		if (oEvent && oEvent.target && oEvent.target.id) {
			oTarget = Element.getElementById(oEvent.target.id);
		}
	...
	ProcessFlow.prototype.onsaptabnext = function (oEvent) {
		...
		var oActiveElement = Element.getElementById($activeElement.id);
		if (!oActiveElement && $activeElement && $activeElement.childElementCount > 0) {
			$activeElement = $activeElement.childNodes[0];
			oActiveElement = Element.getElementById($activeElement.id);
		}
	...
	ProcessFlow.prototype._onArrowClick = function (oEvent) {
		var sTargetId = oEvent.target.id || oEvent.srcControl.getId();
		if (sTargetId) {
			var sId = this.getId();
			...
			for (var j = 0; j < oCurrentNodeChildren.length; j++) { // Check the children.
				...
				if (oNodeChild) {
					if (oCurrentNode.getLaneId() === oNodeChild.getLaneId()) {
						...
						// we are creating the id using "#", so that the id of this lane will not collide with the upcoming predefined lanes
						iNewLaneId = oNodeChild.getLaneId() + "#";
	...
	ProcessFlow.prototype._registerMouseWheel = function (oEvent) {
		...
		var fnDoNotListen = function () {
			var iDiffTime = Date.now() - this._wheelTimestamp;
	...
	ProcessFlow.prototype._getScrollingArrow = function (direction) {
		...
		if (direction == 'left') {
			 sAltText = this._resBundle.getText("PF_ARROW_ICON_LEFT");
		} else {
			 sAltText = this._resBundle.getText("PF_ARROW_ICON_RIGHT");
		}
		...
		var mProperties = {
			...
			tooltip: direction === ProcessFlow._constants.left ? this._resBundle.getText("PF_ARROW_ICON_LEFT") : this._resBundle.getText("PF_ARROW_ICON_RIGHT")
			};
