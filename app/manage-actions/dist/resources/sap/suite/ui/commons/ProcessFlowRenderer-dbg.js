...
sap.ui.define([
	...
	"sap/ui/core/Core",
	"sap/ui/core/Lib"
], function (jQuery, library, ProcessFlowLaneHeader, AriaProperties, Device, encodeXML, Element, Core, CoreLib) {
	"use strict";
	...
	ProcessFlowRenderer._renderNormalNodeHeader = function (oRm, oControl, node, counterLaneNumber, laneNumber) {
		oRm.openStart("th").attr("scope", "colgroup").attr("colspan","3").attr("id", oControl.getId() + "-colgroup" + counterLaneNumber).openEnd();
	...
	ProcessFlowRenderer._renderMergedNodeHeader = function (oRm, oControl, node, counterLaneNumber, laneNodeStates, renderProcessSymbol) {
		...
		oRm.openStart("th").attr("scope", "colgroup").attr("colspan", nCollNumber).attr("id", oControl.getId() + "-colgroup" + nCollNumber).openEnd();
		oRm.renderControl(node);
		oRm.close("th");
	...
	ProcessFlowRenderer._renderTableHeader = function (oRm, oControl, lanePositionNodes, laneNumber) {
		...
		oRm.openStart("th").attr("scope","col").attr("id", oControl.getId() + "-colstart" ).openEnd();
		...
		var aNodeStates = [];
		var sArtificialNodeSuffix = "#"; //Each following artificial node has one more '1' at the end.
		...
		oRm.openStart("th").attr("scope","col").attr("id", oControl.getId() + "-colend" ).openEnd();
	...
	ProcessFlowRenderer._renderBasicStructure = function (oRm, oControl) {
		...
		oRm.attr("aria-roledescription", "Process Flow");
	...
	ProcessFlowRenderer._setLineTypeInLabel = function (connectionEntries, selectedOrHighlightedWasFound) {
		...
		if (oTargetNode.getHighlighted()) {
			...
			for (var i = 0; i < aParentsOfTargetNode.length; i++) {
				oSourceNode = Element.getElementById(aParentsOfTargetNode[i]);
