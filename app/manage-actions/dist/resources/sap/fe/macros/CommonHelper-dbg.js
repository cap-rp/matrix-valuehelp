...
sap.ui.define(["sap/base/Log", "sap/fe/base/BindingToolkit", "sap/fe/core/CommonUtils", "sap/fe/core/converters/controls/Common/Action", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/MetaModelFunction", "sap/fe/core/helpers/ModelHelper", "sap/fe/macros/filterBar/DefaultSemanticDateOperators", "sap/fe/macros/filterBar/ExtendedSemanticDateOperators", "sap/fe/macros/filterBar/SemanticDateOperators", "sap/m/library", "sap/ui/Device", "sap/ui/mdc/condition/FilterOperatorUtil", "sap/ui/mdc/enums/FieldEditMode", "sap/ui/mdc/odata/v4/TypeMap", "sap/ui/model/Context", "sap/ui/model/odata/v4/AnnotationHelper"], function (Log, BindingToolkit, CommonUtils, Action, MetaModelConverter, MetaModelFunction, ModelHelper, DefaultSemanticDateOperators, ExtendedSemanticDateOperators, SemanticDateOperators, mLibrary, Device, FilterOperatorUtil, FieldEditMode, TypeMap, Context, ODataModelAnnotationHelper) {
  ...
  const CommonHelper = {
    ...
    getMetaModelId: function (target, oInterface) {
    ...
    getTargetCollectionPath: function (context, navCollection) {
      ...
      if (context.getObject("$kind") === "EntitySet" || context.getObject("$ContainsTarget") === true) {
        return sPath;
      }
      const model = context.getModel();
      const metaModel = model.isA("sap.ui.model.odata.v4.ODataMetaModel") ? model : model.getMetaModel();
    ...
    isPropertyFilterable: function (context, oDataField, skipHiddenFilters) {
    ...
    getHeaderDataPointLinkVisibility: function (path, isClickable) {
      return compileExpression(ifElse(constant(isClickable), equal(pathInModel(`isHeaderDPLinkVisible/${path}`, "internal"), true), notEqual(pathInModel(`isHeaderDPLinkVisible/${path}`, "internal"), true)));
    },
    /**
    ...
    getPressHandlerForDataFieldForIBN: function (oDataField, sContext, bNavigateWithConfirmationDialog) {
      ...
      return this.generateFunction(bNavigateWithConfirmationDialog ? "._intentBasedNavigation.navigateWithConfirmationDialog" : "._intentBasedNavigation.navigate", this.addSingleQuotes(oDataField.SemanticObject), this.addSingleQuotes(oDataField.Action), this.objectToString(mNavigationParameters), "${$source>/}");
    },
     ...
     * @private
     */
    getMeasureAttributeIndex: function (iMeasure, oChartAnnotations, isMicroChart) {
      ...
      if (oChartAnnotations?.Measures?.length && oChartAnnotations?.Measures?.length > 0) {
        ...
        sMeasurePropertyPath = isMicroChart ? aMeasures[iMeasure].value : aMeasures[iMeasure].$PropertyPath;
      ...
      const fnCheckMeasure = function (sMeasurePath, oMeasureAttribute, index) {
        if (oMeasureAttribute) {
          const path = isMicroChart ? oMeasureAttribute.Measure?.value : oMeasureAttribute.Measure?.$PropertyPath;
    ...
    getMeasureAttribute: async function (oContext) {
    ...
    getParameters: function (sPath, metaModel) {
      if (metaModel) {
    ...
    buildActionWrapper: function (oAction, oThis) {
      ...
      if (oThis && oThis.id) {
        ...
        const oAdditionalParams = {
          contexts: ref(internalModelPath)
        };
     ...
     */
    getOperatorsForProperty: function (sProperty, sEntitySetPath, oContext, sType, bUseSemanticDateRange, settings) {
    ...
    getOperatorsForDateProperty: function (propertyType) {
