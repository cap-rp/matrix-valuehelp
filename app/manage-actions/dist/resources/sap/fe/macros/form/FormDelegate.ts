...
const Delegate = {
	...
	createLayout: async function (mPropertyBag: {
		...
		let sEntitySet = await DelegateUtil.getCustomDataWithModifier<string>(oForm, "entitySet", oModifier);
		if (!sEntitySet) {
			sEntitySet = await DelegateUtil.getCustomDataWithModifier<string>(oForm, "navigationPath", oModifier);
		}
