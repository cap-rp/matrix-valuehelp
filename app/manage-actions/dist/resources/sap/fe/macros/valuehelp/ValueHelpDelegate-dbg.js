...
sap.ui.define(["sap/base/Log", "sap/base/util/ObjectPath", "sap/base/util/deepEqual", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/collaboration/CollaborationCommon", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/TypeGuards", "sap/fe/macros/filter/FilterUtils", "sap/fe/macros/internal/valuehelp/ValueListHelper", "sap/m/inputUtils/highlightDOMElements", "sap/ui/mdc/ValueHelpDelegate", "sap/ui/mdc/condition/Condition", "sap/ui/mdc/enums/ConditionValidated", "sap/ui/mdc/odata/v4/TypeMap", "sap/ui/mdc/p13n/StateUtil", "sap/ui/model/FilterType", "../internal/valuehelp/AdditionalValueHelper"], function (Log, ObjectPath, deepEqual, CommonUtils, CollaborationCommon, MetaModelConverter, TypeGuards, FilterUtils, ValueListHelper, highlightDOMElements, ValueHelpDelegate, Condition, ConditionValidated, TypeMap, StateUtil, FilterType, AdditionalValueHelper) {
  ...
  return Object.assign({}, ValueHelpDelegate, {
    apiVersion: 2,
    updateBindingDonePromises: new WeakMap(),
    /**
     ...
     */
    _getValueHelpHistoryService(control) {
     ...
     */
    _getHistoryData: async function (payload, control) {
     ...
     */
    _setHistoryData: async function (payload, conditions, control) {
     ...
     */
    convertConditionPayloadToNestedObject: function (conditionPayload) {
      const nestedObject = {};
    ...
    updateBinding: async function (valueHelp, listBinding, bindingInfo, content) {
      ...
      if (content.isTypeahead()) {
        ...
        this.updateBindingDonePromises.set(listBinding, existingUpdateBindingDonePromise ? existingUpdateBindingDonePromise.then(async () => newPromise) : newPromise);
        const valueListProperty = this._getValueListPropertyFromPayloadQualifier(payload);
        const field = content.getControl();
        const isFilterFieldOrMultiValuedField = field.isA("sap.ui.mdc.FilterField") || field.isA("sap.ui.mdc.MultiValueField");
        ...
        const isDigitSequence = field.getModel().getMetaModel().getObject(`${payload.propertyPath}${AnnotationIsDigitSequence}`) ?? false;
        if (!fieldValue || field.hasPendingUserInput() || isDigitSequence && fieldValue === "0") {
        ...
        const sortersCustomData = content.getTable().getCustomData().filter(data => data.getKey() === "sorters");
        let sorters = [];
        if (sortersCustomData.length) sorters = sortersCustomData[0].getValue().sorters;
        await this._updateBindingWithTransientContexts(payload, listBinding, bindingInfo, additionalValues, valueListProperty, field, sorters);
     ...
     */
    checkListBindingPending: async function (_valueHelp, listBinding, requestedItems) {
      let isPending = false;
     ...
     */
    retrieveContent: async function (valueHelp, container, contentId) {
      const payload = valueHelp.getPayload();
    ...
    _onConditionPropagationToFilterBar: async function (conditions, outParameters, filterBar, payload, listReportFilterBar) {
      try {
        ...
        const filterBarProperties = FilterUtils.getFilterPropertyInfo(filterBar);
        ...
        for (const outParameter of outParameters) {
          const filterBarProperty = this._findFilterBarPropertyByAnnotationPath(outParameter, payload.propertyPath, metaModel, filterBarProperties);
          if (filterBarProperty && stateItems?.find(item => item.name === filterBarProperty.name)) {
            ...
            const conditionPath = filterBarProperty.conditionPath;
    ...
    _findFilterBarPropertyByAnnotationPath: function (outParameter, propertyPath, metaModel, filterBarProperties) {
    ...
    _updatePropertyViaOutParameter: function (convertedMetadata, rootPath, outValues, outParameter, context, contextCanRequestSideEffects) {
      ...
      const textPath = isPathAnnotationExpression(targetProperty?.annotations?.Common?.Text) ? targetProperty?.annotations?.Common?.Text.path : undefined;
     ...
     */
    getFilterConditions: function (valueHelp, content, config) {
    ...
    onConditionPropagation: async function (valueHelp, reason, _config) {
      ...
      const listReportFilterBar = field.getParent(); // Control e.g. FormContainer
      if (listReportFilterBar.isA(FeCoreControlsFilterBar) && outParameters.length && conditions.length) {
        ...
        if (context) {
          const metaModel = field.getModel().getMetaModel();
     ...
     */
    _createInitialFilterCondition: function (value, initialValueIsSignificant) {
      let condition;
      if (value === undefined) {
    ...
    _getInitialFilterConditionsFromBinding: async function (inConditions, control, inParameters, payload) {
      ...
      */
      ...
      if (control.isA("sap.ui.mdc.MultiValueField") && navigationProperty && navigationProperty.$Partner === undefined) {
        ...
        Log.error(`ValueHelpDelegate: Because of missing partner definition, the filter value for ValueListParameterIn property can not requested.`);
        metaDataError = true;
      ...
      if (!metaDataError) {
        ...
        const contextUnderCreation = bindingContext.isTransient() === true && !bindingContext.isInactive() && bindingContext.getUpdateGroupId()?.startsWith("$auto");
        // We await creation if it is still in process.
        if (contextUnderCreation) {
    ...
    _buildPropertiesToRequest: function (propertiesToRequest, rootPath, metaModel, propertyPath) {
      /*
      ...
      let metaModelContext = metaModel.createBindingContext(rootPath);
      const relativePathParts = propertyPath.slice(rootPath.length) // remove root path
      ...
      .map(part => {
        ...
        metaModelContext = metaModel.createBindingContext(part, metaModelContext);
        return {
          part: part,
      ...
      return propertiesToRequest.map(localDataPath => {
        ...
        const matchCount = localDataParts.findIndex((part, index // find first non-matching index
        ) => relativePathParts[-index]?.partner !== part); // relativePathParts to match from the end (=> "-index") to localDataParts from the beginning
        ...
        return relativePathParts.slice(0, -matchCount) // remove all matched parts (from the end)
        .map(part => part.part).concat(localDataParts.slice(matchCount)) // remove the matched parts from localData (from the beginning)
        .join("/"); // transfer back to string (but only once at the end ;) )
      });
    },
    _getInitialFilterConditionsFromFilterBar: async function (inConditions, control, inParameters, payload) {
      ...
      for (const inParameter of inParameters) {
        const filterBarProperty = this._findFilterBarPropertyByAnnotationPath(inParameter, payload.propertyPath, metaModel, filterBarProperties);
        const sourceField = filterBarProperty ? filterBarProperty.conditionPath : inParameter.source.split("conditions/").pop();
        const conditions = this._getConditionsFromInParameter(sourceField, state);
        if (conditions) {
     ...
     * @since 1.114.0
     */
    _getConditionsFromInParameter: function (sourceField, state) {
    ...
    getInitialFilterConditions: async function (valueHelp, content, control) {
      ...
      for (const inParameter of inParameterMap.constant) {
        const condition = this._createInitialFilterCondition(inParameter.constantValue, isObjectPage ? inParameter.initialValueIsSignificant : false // no filter with "empty" on ListReport
        );
        if (condition) {
          inConditions[inParameter.helpPath] = [condition];
        }
     ...
     */
    findConditionsForContext: function (valueHelp, content, context, conditions) {
      ...
      const selectedCondition = conditions.find(function (condition) {
        if (context?.getObject(keyPath) === condition?.values[0]) {
          const conditionPayloadMap = condition.payload;
     ...
     */
    _updateBindingWithTransientContexts: async function (payload, listBinding, bindingInfo, additionalValues, valueListProperty, field, sorters) {
      try {
        ...
        } = await this._getRelevantAdditionalValueContextData(additionalValues, listBinding, bindingInfo, payload, valueListProperty, field, sorters);
        ...
        additionalValueHelper.createTransientContextsForAdditionalValueContextData(recommendationValuesContextData, recentValuesContextData, othersValuesContextData, listBinding);
      } catch (error) {
     ...
     * @param sorters Sorters of the value help list binding
     */
    _updateBindingAttributes: async function (listBinding, bindingInfo, sorters) {
     ...
     * @param field Control connected to value help
     * @param sorters Sorters of the value help list binding
     * @returns Object which contains recommendation, recently used and others context data.
     */
    _getRelevantAdditionalValueContextData: async function (additionalValues, listBinding, bindingInfo, payload, valueListProperty, field, sorters) {
      let {
        recommendationValuesContextData,
        othersValuesContextData
      } = await additionalValueHelper.requestForAdditionalValueContextData(additionalValues, listBinding, bindingInfo, payload, sorters, field);
      ...
      recommendationValuesContextData = additionalValueHelper.getRelevantRecommendationValuesContextData(recommendationValuesContextData, additionalValues[0]?.values, valueListProperty);
      let recentValuesContextData = [];
      if (!payload.isValueListWithFixedValues) {
        ...
        recentValuesContextData = additionalValueHelper.getRelevantRecentValuesContextData(othersValuesContextData, recentValuesContextData, valueHelpKeys, bindingInfo.parameters.$search?.replaceAll('"', ""), bindingInfo);
      }
      othersValuesContextData = additionalValueHelper.getRelevantOthersValuesContextData(recommendationValuesContextData, recentValuesContextData, othersValuesContextData, valueHelpKeys);
      return {
        recommendationValuesContextData,
        recentValuesContextData,
        othersValuesContextData
     ...
     * @param bindingInfo The binding info object to be used to bind the list to the model
     */
    _updateBinding: function (listBinding, bindingInfo) {
      const rootBinding = listBinding.getRootBinding() || listBinding;
      if (!rootBinding.isSuspended()) {
        rootBinding.suspend();
      }
     ...
     */
    _checkIfAdditionalValuesExists: async function (content, payload) {
      let checkRecommendations = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
     ...
     */
    getRelevantRecommendations(content, payload) {
      let values = [];
      ...
      if (!isFilterFieldOrMultiValuedField) {
        const contentModel = content.getModel();
        ...
        if (!values.length) {
          ...
          values = additionalValueHelper.getRelevantRecommendations(inputValues, bindingContext, payload.propertyPath, content.getControl().getBinding("value")?.getPath()) || [];
        }
      }
      return values;
    },
     ...
     */
    showTypeahead: async function (valueHelp, content) {
      let showIt = true;
      const filterValue = content?.getFilterValue();
      const field = content.getControl();
      const payload = valueHelp.getPayload();
      if (field.isA("sap.ui.mdc.FilterField") || field.isA("sap.ui.mdc.MultiValueField")) {
        if (!filterValue) {
          // according to the current recommendation UX we don't check for recommendations but recently entered values only
          showIt = await this._checkIfAdditionalValuesExists(content, payload, false);
        }
      } else {
        ...
        if (fieldValue || fieldTextValue) {
          ...
          const isDigitSequence = field.getModel().getMetaModel().getObject(`${payload.propertyPath}${AnnotationIsDigitSequence}`) ?? false;
          if (field.hasPendingUserInput() || isDigitSequence && fieldValue.match(/^0+$/)) {
            showIt = filterValue !== "" || (await this._checkIfAdditionalValuesExists(content, payload));
          } else {
            // check recently used only
            showIt = await this._checkIfAdditionalValuesExists(content, payload, false);
          }
        } else if (!filterValue) {
          showIt = await this._checkIfAdditionalValuesExists(content, payload);
          // else: the user types, we shall always show the other values
        }
      }
      return showIt;
    },
    shouldOpenOnFocus: async function (valueHelp, content) {
      try {
        const parentControl = valueHelp.getControl();
        if (parentControl && parentControl?.isA("sap.ui.mdc.FilterField")) {
        ...
        const valueHelpTable = content?.getContent()[0];
        let control;
        const getParentDialog = function (innerContent) {
          control = innerContent?.getParent();
          if (control && control.isA("sap.m.Dialog")) {
            return control;
        ...
        if (control && control.data("fe_InitialTypeAheadFocusDone") === null && content.getControl().getDomRef()?.getElementsByTagName("input")[0].id === control.getInitialFocus()) {
...
//# sourceMappingURL=ValueHelpDelegate-dbg.js.map
