...
sap.ui.define(["sap/base/Log", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/editFlow/draftDataLossPopup", "sap/fe/core/controllerextensions/messageHandler/messageHandling", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/helpers/TypeGuards", "sap/m/Button", "sap/m/Dialog", "sap/m/MessageBox", "sap/m/Text", "sap/ui/core/Lib", "../../helpers/ModelHelper", "./operations/Operation"], function (Log, CommonUtils, draftDataLossPopup, messageHandling, MetaModelConverter, ResourceModelHelper, TypeGuards, Button, Dialog, MessageBox, Text, Library, ModelHelper, Operation) {
   ...
   */
  function getConvertedAction(context, operation) {
    ...
    return convertTypes(metaModel).resolvePath(boundAction.getPath()).target;
  }
   ...
   */
  async function executeDraftEditAction(oContext, bPreserveChanges, oView, groupId, messageHandler, overwriteChange) {
    ...
    if (convertedAction) {
      const operation = new Operation(CommonUtils.getAppComponent(oView), oContext.getModel(), convertedAction, {
        ...
        oDataProperties: {
          ...
          ignoreETag: oContext.getObject() === undefined,
      ...
      operation.setDefaultParametersValues({
        PreserveChanges: bPreserveChanges
      });
      const result = await operation.execute();
      if (result[0].status === "rejected") {
        const modifiedError = new Error(result[0].reason);
   ...
   * @returns Resolve function returns the context of the operation
   */
  async function executeDraftActivationAction(oContext, oAppComponent, messageHandler, sGroupId, oPreparePromise) {
   ...
   */
  function getMessagePathForPrepare(context) {
   ...
   * @returns Path to the message
   */
  function getMessagePathForContext(context) {
   ...
   * @returns Resolve function returns the context of the operation
   */
  async function executeDraftPreparationAction(oContext, groupId, bMessages, ignoreETag) {
    if (!oContext.getProperty("IsActiveEntity")) {
      ...
      return oOperation.invoke(sGroupId, ignoreETag).then(function () {
        return oOperation;
   ...
   */
  async function requestMessages(oContext, oSideEffectsService) {
    let removeTransitionMessages = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
   ...
   * @returns Resolve function returns the context of the operation
   */
  async function executeDraftDiscardAction(oContext, messageHandler, oAppComponent, bEnableStrictHandling) {
    if (!oContext.getProperty("IsActiveEntity")) {
   ...
   */
  async function computeSiblingInformation(rootCurrentContext, rightmostCurrentContext, rootContextInfo, groupId) {
    ...
    try {
      ...
      const canonicalPathPromises = paths.map(async segment => {
        ...
        if (currentPath.endsWith(")")) {
          const siblingContext = model.bindContext(`${currentPath}/SiblingEntity`, undefined, groupId ? {
            $$groupId: groupId
          } : undefined).getBoundContext();
      ...
      if (rootContextInfo?.rootSiblingPathPromise) {
        const rootSiblingPath = (await rootContextInfo.rootSiblingPathPromise)?.getPath();
   ...
   */
  async function createDraftFromActiveDocument(oContext, oAppComponent, mParameters, groupId, messageHandler) {
     ...
     */
    async function overwriteChange(response) {
    ...
    try {
      ...
      if (oResponse.status === 409 || oResponse.status === 423) {
        messageHandling.removeBoundTransitionMessages();
        messageHandling.removeUnboundTransitionMessages();
        ...
        if (siblingInfo?.targetContext) {
          ...
          oDraftContext = await overwriteChange(oResponse);
        }
  ...
  async function activateDocument(oContext, oAppComponent, mParameters, messageHandler) {
    ...
    if (!hasPrepareAction(oContext)) {
      ...
      const ignoreEtag = oContext.getModel().hasPendingChanges("$auto") ? true : false;
   ...
   */
  async function showEditConfirmationMessageBox(sUnsavedChangesMsg, oContext) {
   ...
   */
  async function deleteDraft(oContext, messageHandler, oAppComponent, bEnableStrictHandling) {
    ...
    if (bIsActiveEntity || !bIsActiveEntity && !sDiscardAction) {
      ...
      if (oContext.hasPendingChanges() && !oContext.isInactive()) {
        ...
        return !oContext.getProperty("IsActiveEntity") && ModelHelper.isDraftRoot(getInvolvedDataModelObjects(oContext.getModel().getMetaModel().getMetaContext(oContext.getPath()))?.targetEntitySet) && oContext.getProperty("HasActiveEntity") ? oContext.getModel().delete(oContext) : oContext.delete();
      }
    } else {
...
//# sourceMappingURL=draft-dbg.js.map
