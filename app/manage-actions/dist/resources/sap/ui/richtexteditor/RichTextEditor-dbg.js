...
sap.ui.define([
	...
	var RichTextEditor = Control.extend("sap.ui.richtexteditor.RichTextEditor", /** @lends sap.ui.richtexteditor.RichTextEditor.prototype */ {
		metadata: {
			...
			properties: {
				 ...
				 */
				editorLocation: { type: "string", group: "Misc", defaultValue: 'js/tiny_mce6/tinymce.js', deprecated: true },
	...
	RichTextEditor.prototype._setupToolbar = function () {
		 ...
		 * @deprecated As of version 1.120
		 */
		 mapping["TinyMCE5"] = fnMapPluginList(RichTextEditor.DEFAULT_PLUGINS_TINYMCE5);
		 ...
		 * @deprecated As of version 1.120
		 */
		fontButtonGroupButtoNamesMapping["TinyMCE5"] = ["fontselect", "fontsizeselect", "forecolor", "backcolor"];
	 ...
	 *
	 * @param {string} [sEditorType] Which editor type to be used (currently only 5 and 6)
	 * @returns {sap.ui.richtexteditor.RichTextEditor} Control instance (for method chaining)
	 * @public
	 */
	RichTextEditor.prototype.setEditorType = function(sEditorType) {
		...
		if (!this._bEditorCreated) { // only supported before first rendering!
			...
			if (oEditorMapping[sEditorType] === "TinyMCE5") {
				Log.warning("TinyMCE version 5 is used as editor type. TinyMCE 5 is depreacated and will be removed from the code entirely in future.");
			}
	 ...
	 * @returns {string[]|string} Specific button format. Returns a single string for TinyMCE 5
	 * @private
	 */
	...
	RichTextEditor.prototype._shouldLoadTinyMCE = function () {
		 ...
		 * @deprecated As of version 1.120
		 */
		if (sEditorType === RichTextEditor.EDITORTYPE_TINYMCE5) {
	...
	RichTextEditor.prototype.onBeforeRenderingTinyMCE = function() {
		if (this._shouldLoadTinyMCE()) {
			var sEditorLocation = EDITOR_LOCATION_MAPPING[this.getEditorType()];
	...
	RichTextEditor.prototype._onAfterReadyTinyMCE = function() {
		...
		this._oEditor.on("change", (oEvent) => this.onTinyMCEChange(this._oEditor)); // Works for TinyMCE 5

		this._oEditor.on("focusout", (oEvent) => this.onTinyMCEChange(this._oEditor));
	...
	RichTextEditor.prototype._getLanguageTinyMCE = function() {
		...
		// If there is region language in the additional languages supported in TinyMCE6, return it
		if (RichTextEditor.SUPPORTED_LANGUAGES_TINYMCE6[sLangStr]) {
			return sLangStr;
		}

		// If there is language in the additional languages supported in TinyMCE6, return it
		if (RichTextEditor.SUPPORTED_LANGUAGES_TINYMCE6[sLanguage]) {
			return sLanguage;
		}
