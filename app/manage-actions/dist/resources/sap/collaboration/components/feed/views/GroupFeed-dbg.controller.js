...
sap.ui.define([
	...
	function(jQuery, Log, encodeURL, isEmptyObject, Controller, TimelineItem, MessageBox, LanguageBundle, DateUtil, FeedEntryEmbedded, ReplyPopover, SocialTextArea, FilterPopover, CommonUtil, ModeFactory, JSONModel, ToolbarSpacer, Button, mobileLibrary, Fragment, ResponsivePopover, Device, CustomData, Link, MessageType, List, CustomListItem, library, Component) {
	"use strict";
	...
	var oGroupFeed =  Controller.extend(sControllerName, {
		 ...
		 * @memberOf sap.collaboration.components.feed.views.GroupFeed
		 */
		_initializeTimeline: async function() {
			var oTimeline = this.byId("timeline");
			oTimeline.setContent([]);

			await this._modifyHeaderBar();
		 ...
		 * @memberOf sap.collaboration.components.feed.views.GroupFeed
		 */
		_modifyHeaderBar: async function(){
			...
			oHeaderBar.insertContent(oSpacer, 1);
				...
				var oCustomListItem = await Fragment.load({
					name: "sap.collaboration.components.feed.fragments.CustomListItem",
					controller: this}
					);
		 ...
		 * @memberOf sap.collaboration.components.feed.views.GroupFeed
		 */
		_createTimelineItem: async function(oFeedEntry) {
			...
			await oReplyPopover.fullyCreated();
		 ...
		 * @memberOf sap.collaboration.components.feed.views.GroupFeed
		 */
		_addFeedEntriesToTimeline: async function(feedEntries) {

			// create timeline item controls for each feed entry and add it to the timeline
			var oTimeline = this.byId("timeline");

			for (const oFeedEntry of feedEntries) {
				var oTimelineItem = await this._createTimelineItem(oFeedEntry);
		 ...
		 * @memberOf sap.collaboration.components.feed.views.GroupFeed
		 */
		_processFeedEntries: async function(feedEntries) {
			if (feedEntries.length > 0) {
				await this._addFeedEntriesToTimeline(feedEntries);
			}
			else {
		 ...
		 * @memberOf sap.collaboration.components.feed.views.GroupFeed
		 */
		_createSocialProfile: async function(){
			this._pSocialProfile = Component.create({
				name: "sap.collaboration.components.socialprofile"
			});
			this._oSocialProfile = await this._pSocialProfile;
		...
		onAddPost: function(event) {
			...
			if (sContent && sContent.trim() !== "") {
				this._getLoggedInUser().done(function() {
					...
					var fnSuccessCallback = async function (oData, response) { // success callback
		...
		_loadFeedEntriesSuccess: function(oData, response) {
			...
			return this._processFeedEntries(aFeedEntries);
		},
		/**
